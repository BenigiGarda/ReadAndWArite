...
<table align="left" width="20%" class="tabletype" border="1">
        <tr align="center">
                <th>Number</th> <th>Stationary Type Name</th>      
        </tr>
        @foreach($data as $item)
        <tr align="center">
            <td> {{$item->id}} </td> <td>{{$item->type_name}}</td>
        </tr>
        @endforeach
    </table>
    <br><br>

    <form action="/adminhomeonlogin/addtype/store" method="GET" class="form">
       <pre>  <b> browse photos </b> <input type="file" name="file"><br> </pre>
    <input type="text" class="formposisi1" name="Stasionary" value="{{ old('cari') }}" ><br>
    <input type="submit" class="formposisi" value="add New Stationary Type">   
